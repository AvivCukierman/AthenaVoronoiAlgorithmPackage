...
MyCompletePackageAlg::MyCompletePackageAlg( const std::string& name, ISvcLocator* pSvcLocator ) : AthAnalysisAlgorithm( name, pSvcLocator ){
...
StatusCode MyCompletePackageAlg::initialize() {
  ...
  m_JetConstituentModSequence.setTypeAndName("JetConstituentModSequence/MyJetConstituentModSequence");
...
StatusCode MyCompletePackageAlg::execute() {  
  ...
  std::string m_outputContainer = "VoronoiClusters";
  const xAOD::CaloClusterContainer*             voronoi_clusters   (nullptr);
  if(evtStore()->retrieve(voronoi_clusters,m_outputContainer).isFailure()) ATH_MSG_ERROR("Could not retrieve the voronoi cluster container");
  for(auto clust: *voronoi_clusters){
    std::cout << clust->pt() << std::endl;
    break;
  }
